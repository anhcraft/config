...
- `@Name`: set the property primary name and aliases
  ...
  - A name is valid if it is non-empty and not conflicted with previous name/aliases. If conflict exists, the name is implicitly discarded.
...
- `@Alias`: set the property aliases
  ...
  - It is similar to defining aliases using `@Name` except that:
    - Using `@Name`, the first valid name is the primary
    - Using `@Alias`, the primary name is generated by default (if `@Name` is not specified)
...
- `@Restrict`: restricts the setting value
  - The difference between using `@Restrict` and a separate validation is that `@Restrict` integrates well with the ConfigDoc
  - Throws exception when violation occurs, and can be discarded using `silent`
...
### Value restriction
- For example: `@Restrict("not-null, not-empty, range=3|5")`
...
- The denormalization is much complex than the normalization:
...
- The adapter works with the schema, the POJO and the wrapper.
