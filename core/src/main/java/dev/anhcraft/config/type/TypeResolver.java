...
public abstract class TypeResolver implements Type {
  ...
  public static @NotNull TypeResolver of(@NotNull Type type) {
    if (type instanceof TypeResolver) return (TypeResolver) type;
   ...
   */
  @NotNull public Map<String, Type> getTypeMapping() {
    if (typeMapping != null) return typeMapping;
    if (!(provideType() instanceof ParameterizedType)) return Map.of();
    Type rawType = ((ParameterizedType) provideType()).getRawType();
    if (!(rawType instanceof Class)) return Map.of();
